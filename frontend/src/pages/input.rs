...
use yew::prelude::*;
use yew_hooks::{use_list, use_mount};
...
pub fn input() -> Html {
    ...
    let date_is_incomplete = (!(*session).today_selected())
        .then_some(0)
        ...
        .map(|required| {
            local_diary_entry
                .current()
                .iter()
                .any(|v: &DiaryEntry| required.contains(&v.practice) && v.value.is_none())
        })
        .unwrap_or(false);
    ...
    {
        let layout = layout.clone();
        use_effect_with(date_is_incomplete, move |incomplete| {
            layout.set_selected_day_incomplete(*incomplete);
            || ()
        });
    }
